...
export const VerificationDocSchema = z.object({
  name: z.string().min(1, 'Document name is required').max(80, 'Keep it under 80 characters'),
  ...
  files: z
    ...
    .refine((files) => files.every((f) => ALLOWED_MIME_TYPES.includes(f.type as any)), {
...
export const VerificationFormSchema = z
  .object({
    documents: z.array(VerificationDocSchema).min(1, 'Add at least one document')
  })
...
export const useVerificationStore = create<VerificationState>((set, get) => ({
  ...
  addEmptyRow: () =>
    set((state) => ({
      documents: [...state.documents, { name: '', document_type: 'other', description: '', files: [] }]
    })),
  ...
  validate: () => {
    const state = get()
    const parsed = VerificationFormSchema.safeParse({ documents: state.documents })
    if (parsed.success) {
      set({ errors: {} })
      return true
    }
  ...
  reset: () => set({ documents: [{ name: '', description: '', files: [] }], errors: {} })
}))
