...
export function VerificationModal({ onSubmit }: VerificationModalProps) {
  ...
  const submit = async () => {
    ...
    if (flatFiles.length > 5) {
      setGeneralError('You can upload a maximum of 5 verification documents per submission.')
      return
    }
    ...
    try {
      ...
      for (const doc of documents) {
        ...
        for (const f of doc.files) {
          await usersApi.uploadTutorDocument(f, docType, doc.description || doc.name)
        }
      }
  ...
  return (
    <Modal isOpen={isOpen} onClose={close} size="lg" className="p-0">
      <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-200">
        <h3 className="text-lg font-semibold">Verify your account with documents</h3>
        <button onClick={close} className="p-2 rounded-md hover:bg-neutral-100">
          <X className="w-5 h-5 text-neutral-600" />
        </button>
      </div>

      <div ref={containerRef} className="max-h-[60vh] overflow-y-auto px-6 py-4 space-y-4">
        {generalError && (
        ...
        {documents.map((doc, idx) => (
          <div key={idx} className="border border-neutral-200 rounded-lg p-4">
            ...
            {errors[`documents.${idx}.name`] && (
              <p className="text-sm text-red-600 mt-1">{errors[`documents.${idx}.name`]}</p>
            )}
            ...
            >
              ...
              <div className="text-sm text-neutral-700">Drop files here, or click to browse</div>
              <input
                type="file"
                multiple
            ...
            {errors[`documents.${idx}.files`] && (
              <p className="text-sm text-red-600 mt-1">{errors[`documents.${idx}.files`]}</p>
            )}
              ...
              <div className="mt-3 max-h-32 overflow-y-auto border border-neutral-200 rounded-md">
                {doc.files.map((f, i) => (
                  <div key={i} className="flex items-center justify-between px-3 py-2 border-b last:border-b-0">
                    <span className="text-sm text-neutral-800 truncate mr-3">{f.name}</span>
