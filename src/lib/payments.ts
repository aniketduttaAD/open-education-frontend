...
async function createOrder(body: CreateOrderBody) {
  ...
  const { data } = await api.post<
    ...
    | { success: boolean; data: { success: boolean; message?: string; data: CreateOrderResponse } }
  >(
...
async function getUserOrders(range: 'week' | 'month' | 'year' | 'all', sort: 'asc' | 'desc') {
...
async function getTutorPayments(range: 'week' | 'month' | 'year' | 'all', sort: 'asc' | 'desc') {
